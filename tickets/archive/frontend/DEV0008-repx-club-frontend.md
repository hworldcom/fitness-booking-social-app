...
- [Demo domain transitions](../../../src/lib/demo.ts) restrict simulated booking to the seeded membership and prevent duplicates. Cancellation changes the same activity and hiding survives cancellation. [Browser storage](../../../src/lib/store.ts) persists follows, bookmarks, drafts and bookings under the versioned `repx-club-preview-v1` key; malformed data resets safely and unavailable storage falls back to memory with a visible warning. Its availability flag is part of the subscribed snapshot so that the warning appears even when initial demo data is unchanged.
...
- [Fixtures](../../../src/lib/fixtures.ts): added required `ClubClass.dateISO` values for the three existing September 2026 dates and explicit `ClubStudio` entries. Existing prices, membership coverage, class IDs and booking behavior are unchanged. [Filter module](../../../src/lib/explore.ts) owns pure class/studio filtering and shared time/activity options; comparison uses normalized local date and HH:mm strings, independent of the browser's timezone.
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [events domain](../../../src/lib/events.ts)                                                                                                                                                                                                  | Seed event, typed local drafts, schedule/price/capacity/inclusion validation and display conversion.                                                       |
| [demo state](../../../src/lib/demo.ts)                                                                                                                                                                                                       | Event-draft save/delete/reset and backwards-compatible local-store hydration. Malformed event drafts are cleared without dropping valid existing bookings. |
| [Explore filter domain](../../../src/lib/explore.ts)                                                                                                                                                                                         | Event search/activity/date/time intersection, reusing time boundaries.                                                                                     |
...
| [Challenge fixtures](../../../src/lib/fixtures.ts), [challenge UI](../../../src/features/challenges/challenges.tsx), [shared cards](../../../src/components/ui.tsx)                                                                          | Explicit award/rules model and winner explanation, plus link to noncompetitive events.                                                                     |
