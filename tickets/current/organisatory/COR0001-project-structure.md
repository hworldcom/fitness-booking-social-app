...
The current app has thin route files under `src/app`, capability-owned screens under `src/features`, reusable interface modules under `src/components`, and still-mixed fixture/local-preview logic under `src/lib`. There is no `src/domain`, `src/server`, `src/solana`, `supabase`, or `programs` implementation yet. This coordination record owns the cross-ticket structure map only; peer development tickets own actual feature behavior, dependencies, migrations and program code.
...
| `src/lib`                                   | Present and still mixes pure contracts/rules, discovery operations, preview fixtures and browser storage.      | [DEV0031 — Preview data and domain boundaries](../frontend/DEV0031-preview-data-and-domain-boundaries.md).                                                                                                                                                              |
| `src/domain`, `src/features/preview`        | Not yet present.                                                                                               | Created from real `src/lib` modules only by DEV0031.                                                                                                                                                                                                                    |
...
| Pure domain, discovery and preview separation | [DEV0031 — Preview data and domain boundaries](../frontend/DEV0031-preview-data-and-domain-boundaries.md)                    | Owns the exact `src/lib` export moves into `src/domain`, `src/features/discovery` and `src/features/preview`, including import updates and behavior-preservation evidence.            | Follows DEV0030 and prepares stable mappings for database-backed features.                                   |
...
2. Implement DEV0031 as the next frontend structural slice before persistence work replaces preview adapters.
...
2. Preserve DEV0030's delivered route/feature/shared-UI boundary. DEV0031 separately moves preview fixtures/storage, pure domain contracts and discovery operations without reopening completed screen moves.
...
- [ ] AC2: DEV0030's completed route/feature/shared-UI boundary remains stable; DEV0031 delivers pure-domain, discovery and preview separation without changing public routes, fixture values or storage behavior.
...
The DEV0036 architecture refinement changed no source directory, dependency, import, runtime configuration, database, wallet integration, program, application test or deployment. DEV0030 remains the only delivered structural slice; its application changes and evidence stay in that ticket. Link each later development ticket's delivered file map and summarize only cross-ticket integration evidence here.
...
- **Remaining implementation checks — not run:** DEV0030 supplies completed frontend evidence, but DEV0031, DEV0015, DEV0025, DEV0027 and the missing server-Solana/Anchor tickets have not delivered their boundaries. Their application tests, import checks, database checks, wallet rehearsal and program checks remain outstanding, so COR0001's AC1–AC5 are not complete.
