...
**Current status:** the responsive Next.js frontend preview is implemented with typed fixtures and browser-local persistence. Its public description, **How it works** guide and Home/Explore/My Access/Profile navigation now present the access-focused direction; the retired product-challenge routes, fixtures and browser actions have been removed. My Access is deliberately an empty preview because no usable membership, pass or ticket repository exists yet. The local Supabase schema/migration foundation, server-only Drizzle boundary and verified protected-actor context are implemented and validated. Versioned membership-product storage now contains two private, price-pending draft fixtures, but product screens still do not read persistent product data and no catalogue offer is published. Email-code accounts exist, optional personal-wallet linking is in progress and prepared club-wallet authority awaits its real-Phantom completion rehearsal. No usable membership, public persistent catalogue, real EURC payment, sponsored-event settlement or membership program exists yet.
...
The [frontend foundation ticket](tickets/archive/frontend/DEV0008-repx-club-frontend.md) records the original slice. You can currently explore Home, Explore, My Access and Profile, follow demonstration people in preview mode, discover Run & Coffee under Explore → Events, save an event draft in preview mode and open class-pass checkout previews. My Access shows the three planned access families without claiming ownership. Checkout previews create no payment, pass, ticket, reservation or feed activity. Event drafts and follows persist in this browser; **Profile → Reset preview** clears them.
...
- `src/features/`: capability-owned Home/feed, Explore/discovery, My Access, event, class and profile screens.
- `src/domain/`: framework-independent catalogue, event and discovery rules; no browser, network or persistence authority.
