...
MovX Club is a social fitness-access platform. People discover local fitness businesses and experiences, choose memberships, passes or event tickets, participate in person and optionally share verified activity. Fitness businesses gain a public channel and infrastructure to offer programmable access products and community experiences.

The current MVP direction focuses on one programmable membership lifecycle, dated class or visit passes, paid events and sponsored events where a sponsor covers some or all attendee access. Challenge modes, prize pools, voting and reactions are no longer in scope. The remaining social layer is deliberately small: one-way follows and chronological, explicitly shared participation. Payment, entitlement, reservation, attendance and sharing remain separate states.

Solana provides the membership-state and test-payment layer for the hackathon demo. People create application accounts with email and later link a Phantom wallet for wallet-backed actions; financially active fitness businesses use distinct club wallets operated by individually signed-in administrators. Devnet transactions use test EURC for memberships, passes, event tickets, sponsorship funding and eligible returns; test SOL is used only for network/account costs. Product discovery, application identity, permissions and social data stay in the backend.

**Current status:** the responsive Next.js frontend preview is implemented with typed fixtures and browser-local persistence. Its public description and **How it works** guide now explain the access-focused direction, but challenge routes, fixtures and primary navigation from the superseded direction remain until DEV0060 replaces them. [COR0005](tickets/current/organisatory/COR0005-fitness-access-product-pivot.md) coordinates this public-interface transition. The local Supabase schema/migration foundation, server-only Drizzle boundary and verified protected-actor context are implemented and validated, but product screens do not read persistent protected product data. Email-code accounts exist, optional personal-wallet linking is in progress and prepared club-wallet authority awaits its real-Phantom completion rehearsal. No usable membership, persistent access catalogue, real EURC payment, sponsored-event settlement or membership program exists yet.
...
The [frontend foundation ticket](tickets/archive/frontend/DEV0008-repx-club-frontend.md) records the original slice. You can currently explore Feed, Explore, Challenges and Profile, follow demonstration people, discover Run & Coffee under Explore → Events, save an event draft and open class-pass checkout previews. Checkout previews create no payment, pass, reservation or feed activity. Drafts and choices persist in this browser; **Profile → Reset preview** clears them.

These challenge routes are superseded preview implementation, not the target product. The completed DEV0059 public guide describes the target access journey; DEV0060 still owns replacing challenge surfaces with access-focused navigation and honest empty access states. Until then, use the challenge screens only to inspect existing design/system behavior.
...
This is a frontend preview. People, venues, schedules and activity are examples. No current click creates a usable membership, pass, ticket, transfer, reservation, payment or attendance confirmation. Persistent access products, sponsored-event funding and Phantom/Devnet transaction flows remain outstanding; see the [current implementation status](docs/mvp-spec.md) and its acceptance criteria.
...
- `src/features/`: capability-owned Feed, Explore/discovery, legacy challenge, event, class and profile screens. DEV0060 owns the legacy challenge removal and My Access transition.
- `src/domain/`: framework-independent catalogue/event and legacy challenge/discovery rules; no browser, network or persistence authority. This still reflects the superseded preview until DEV0060.
